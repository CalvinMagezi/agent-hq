...
`;
    ...
    async injectIntoCLAUDEMd(workDir, context) {
        ...
        const newContent = existing.includes(MARKER_START)
            ? existing.replace(new RegExp(`${escapeRegex(MARKER_START)}[\\s\\S]*?${escapeRegex(MARKER_END)}`), block)
            : `${block}\n\n${existing}`;
        ...
        return async () => {
            ...
            const cleaned = current
                .replace(new RegExp(`${escapeRegex(MARKER_START)}[\\s\\S]*?${escapeRegex(MARKER_END)}\\n*`), '')
                .trim();
            if (existing) {
    ...
    async injectIntoAGENTSMd(workDir, context) {
